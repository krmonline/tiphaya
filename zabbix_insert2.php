...
function getModel($year,$brand){
  $ch = curl_init();
  $url = "http://uat.tipinsure.com/Motor/getModelCarNewFromFundTable";
...
function getSubModel($year,$brand,$modelName){
  $ch = curl_init();
  $url = "http://uat.tipinsure.com/Motor/getSpecCarNewFromFundTable";
...
function SubmitForm($year,$brand,$modelName,$spec,$insure_type){
  global $cookie;
  $ch = curl_init();
  $url = "http://uat.tipinsure.com/Motor/submit_step_1/";
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
  curl_setopt($ch, CURLOPT_POST, 1);
  ...
  preg_match('/{"success":.*}/mi',$output,$json_arr);
  $cookie = $matches[1];
  if(isset($json_arr)){
    $output = $json_arr[0];
  }else{
...
function step2($insure,$cookie){
  $ch = curl_init();
  $url = "https://uat.tipinsure.com/Motor/motor_step_2/$insure";
  curl_setopt($ch, CURLOPT_URL, $url);
  curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
     ...
     foreach($model_json->data as $modelName){
       ...
       foreach($submodel_json->data as $spec){
	    ...
	    //var_dump($output);
	      ...
	      //preg_match("/\(\'\#package_fund\'\)\.val\(\"([0-9\,]+)\"\)/",$output,$matches);
              //var_dump($matches);
              if($token){
                //2009,TOYOTA,ALPHARD,2.4,first_class,27000.38
                $expectation = (isset($matches[1]))?$matches[1]:"true";
		$hostname = "$each_year-$each_brand-$modelName-$spec-$each_insure";
		$result = hostCreate($token,$hostname);
		//var_dump($result);
		if(isset(json_decode($result)->result->hostids[0])){
			$hostid = json_decode($result)->result->hostids[0];
			$result =  http_create($token,$each_year,$each_brand,$modelName,$spec,$each_insure,$hostid,$expectation);
		}
		$itemname = $each_year.','.$each_brand.','.$modelName.','.$spec.','.$each_insure;
		$exp = '{'.$hostname.':web.test.fail[\\"'.$itemname.'\\"].last()}>0';
		//$exp = "1";
		//echo $hostname."\n";
		//echo $exp."\n";
		$result =  triggerCreate($token,$hostname,$exp);
		var_dump($result);
                //echo $result;
              }
